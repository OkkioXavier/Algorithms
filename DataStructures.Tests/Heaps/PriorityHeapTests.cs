...
public class PriorityHeapTests
{
    ...
    [InlineData(new[] { 10, 10, 1, 1 })]
    [InlineData(new[] { 96, 99, 98, 13, 512, 53121, 54, 1232, 5231, 51312, 5512, 53131, 5120 })]
    [InlineData(new[] { 10, 5, 5, 3, 2, -1, -2 })]
    [InlineData(new[]
    {
    ...
    public void HeapOrderedCorrectly(int[] items)
    {
        var heap = new PriorityHeap<int>(items, (arg1, arg2) => arg1 <= arg2);
        var expectation = items.Order().ToArray();

        for (var i = 0; i < items.Length; i++)
        {
            var item = heap.Pop();
            item.Should().Be(expectation[i]);
        }
    }

    [Fact]
    public void HeapIsCorrectlyShaped()
    {
        int[] items = [10, 5, 5, 3, 2, -1, -2];
        
        _ = new PriorityHeap<int>(items, (arg1, arg2) => arg1 <= arg2);

        items
            .Should()
            .BeEquivalentTo([-2, 2, -1, 3, 5, 10, 5], o => o.WithStrictOrdering());
    }
    ...
    [InlineData(new[] { 10, 10, 9, 9 }, 9)]
    public void PeekTopReturnsCorrectItem(int[] items, int expectation)
    {
        var heap = new PriorityHeap<int>(items, (arg1, arg2) => arg1 <= arg2);

        heap.PeekTop().Should().Be(expectation);
    }

    [Fact]
    public void PopThrowsWhenHeapEmpty()
    {
        var heap = new PriorityHeap<int>([], (arg1, arg2) => arg1 <= arg2);
    ...
    [Fact]
    public void PeekThrowsWhenHeapEmpty()
    {
        var heap = new PriorityHeap<int>([], (arg1, arg2) => arg1 <= arg2);

        var action = () => heap.PeekTop();
